...
public partial class FSpineAnimator_Editor
{
    ...
    void El_DrawAnimatorSetup()
    {
        ...
        bool detected = true;
        if (Get.BaseTransform) if (Get.SyncWithAnimator) detected = FGUI_Finders.CheckForAnimator(Get.BaseTransform.gameObject);
        if (!detected) GUI.color = new Color(.9f, .9f, 0.6f, 1f);
